...
local function makeEdges(nodeRoot, refsFolder, node, draw: boolean?)
    for _, valueRef in refsFolder:GetChildren() do
        ...
        if not refRoot then
            warn(`No Value for connected node {valueRef.Parent.Parent.Name}. Node name: PROBLEM_REF_NIL`)
    ...
    local changed = nodeRoot:GetPropertyChangedSignal("Position"):Connect(function()
        ...
        if node then
            ...
            for _, edge in edges do
                ...
                if not debugPart then continue end
...
CollectionService:GetInstanceAddedSignal(NODE_ROOT_TAG):Connect(function(nodeRoot: BasePart)
    local refsFolder = nodeRoot:FindFirstChild("ConnectedNodes")
...
CollectionService:GetInstanceRemovedSignal(NODE_ROOT_TAG):Connect(function(nodeRoot: BasePart)
